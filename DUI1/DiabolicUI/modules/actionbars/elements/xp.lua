...
if ENGINE_BFA then 
	...
	Bar_Reputation.UpdateData = function(self)
		...
		if standingID then 
			...
			self.data.name = name
			self.data.color = C.Friendship[standingID]
			self.data.barValue = current - min
			self.data.barMax = max - min 
			self.data.standingID = standingID
			self.data.standingLabel = standingLabel
			self.data.isFriend = isFriend
		else 
			self.data.name = nil
			self.data.color = nil
			self.data.barValue = nil
			self.data.barMax = nil
			self.data.standingID = nil
			self.data.standingLabel = nil
			self.data.isFriend = nil
		end 
	...
	Bar_Reputation.OnEnter = function(self)
		...
		local r, g, b = unpack(C.General.Highlight)
		local nameR, nameG, nameB = unpack(data.color)
		GameTooltip:AddDoubleLine(data.name, level, nameR, nameG, nameB, r, g, b)
		GameTooltip:AddDoubleLine(data.standingLabel, longXPString:format(F.Colorize(F.Short(data.barValue), "Normal"), F.Colorize(F.Short(data.barMax), "Normal")), r, g, b, r, g, b)
...
BarWidget.UpdateBarType = function(self)
	...
	local xp, artifact, honor, rep = Module:IsXPVisible()
	local barType = rep and "rep" or honor and "honor" or artifact and "artifact" or xp and "xp" or "none"
