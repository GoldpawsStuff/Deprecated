...
local module = parent:NewModule("Raid25", "GP_AceEvent-3.0")
...
-- WoW strings
local COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE25PLAYERS = _G.COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE25PLAYERS 
...
local function updateConfig()
	T = parent:GetActiveTheme().Raid25
end
...
function module:SpawnFrames()
	oUF:RegisterStyle(addon.."Raid25", Style)
	oUF:SetActiveStyle(addon.."Raid25")
	local T = T
	local db = T.header
	local header = oUF:SpawnHeader(addon.."Raid25", nil, 
	db.showSolo and "solo" or parent:GetVisibilityMacro(16,25), 
...
function module:OnInitialize()
	self.db = parent.db:RegisterNamespace("Raid25", defaults)
	...
	self.fadeManager = LMP:NewChain(gUI4:CreateFadeManager("UnitFrames: Raid25")) :SetFrameStrata("LOW") :SetFrameLevel(100) :Enable() .__EndChain
	self.overlay = gUI4:GlockThis(self.frame, COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE25PLAYERS, function() return module.db.profile end, unpack(gUI4:GetColors("glock", "unitframes"))) 
