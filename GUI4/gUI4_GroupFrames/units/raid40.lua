...
local module = parent:NewModule("Raid40", "GP_AceEvent-3.0")
...
-- WoW strings
local COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE40PLAYERS = _G.COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE40PLAYERS 
...
local function updateConfig()
	T = parent:GetActiveTheme().Raid40
end
...
local function Style(self, unit)
	...
	self.GroupAuras:SetPoint("CENTER", self, 0, 0)
...
function module:SpawnFrames()
	oUF:RegisterStyle(addon.."Raid40", Style)
	oUF:SetActiveStyle(addon.."Raid40")
	local T = T
	local db = T.header
	local header = oUF:SpawnHeader(addon.."Raid40", nil, 
	db.showSolo and "solo" or parent:GetVisibilityMacro(26,40), 
...
function module:OnInitialize()
	self.db = parent.db:RegisterNamespace("Raid40", defaults)
	...
	self.fadeManager = LMP:NewChain(gUI4:CreateFadeManager("UnitFrames: Raid40")) :SetFrameStrata("LOW") :SetFrameLevel(100) :Enable() .__EndChain
	self.overlay = gUI4:GlockThis(self.frame, COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE40PLAYERS, function() return module.db.profile end, unpack(gUI4:GetColors("glock", "unitframes"))) 
