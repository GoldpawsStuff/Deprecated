...
local module = parent:NewModule("Group5", "GP_AceEvent-3.0")
...
-- WoW strings
local COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE5PLAYERS = _G.COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE5PLAYERS 
...
local function updateConfig()
	T = parent:GetActiveTheme().Group5
end
...
local function UpdateStyle(self)
	...
	UpdateFontString(self.CombatFeedbackText, T.fontstrings.combatfeedbacktext)
	
	--	Widgets
	-------------------------------------------------------------------------------
	
	-- auras
	self.Auras.size = T.widgets.auras.auraSize - 2
	self.Auras.spacing = T.widgets.auras.padding + 2
	self.Auras.initialAnchor = T.widgets.auras.initialAnchor
	self.Auras["growth-y"] = T.widgets.auras.growthy
	self.Auras["growth-x"] = T.widgets.auras.growthx
	self.Auras.onlyShowPlayer = T.widgets.auras.onlyShowPlayer
	self.Auras.onlyShort = T.widgets.auras.onlyShort
	self.Auras.num = T.widgets.auras.columns * T.widgets.auras.rows
	self.Auras:SetSize(T.widgets.auras.auraSize*T.widgets.auras.columns + T.widgets.auras.padding*(T.widgets.auras.columns-1), T.widgets.auras.auraSize*T.widgets.auras.rows + T.widgets.auras.padding*(T.widgets.auras.rows-1))
	self.Auras:ClearAllPoints()
	self.Auras:SetPoint(unpack(T.widgets.auras.place))
	self.Auras.disableCooldown = true
	self:EnableElement("Auras")
	if self.Auras.ForceUpdate then
		self.Auras:ForceUpdate()
	end
	if not self._forceUpdateAuras then
		self._forceUpdateAuras = function() 
			if self.Auras.ForceUpdate then
				self.Auras:ForceUpdate()
			end
		end
	end
	self:RegisterEvent("PLAYER_REGEN_DISABLED", self._forceUpdateAuras)
	self:RegisterEvent("PLAYER_REGEN_ENABLED", self._forceUpdateAuras)
...
local function Style(self, unit)
	...
	-- fontstrings
	self.CombatFeedbackText = LMP:NewChain("FontString", nil, self.IconFrame) :SetDrawLayer("ARTWORK") :SetFontObject(NumberFontNormalHuge) .__EndChain
...
function module:ApplySettings()
	...
	for frame in pairs(allFrames) do
		...
		if frame.Auras then
			if self.db.profile.auras.showAuras then
...
function module:SpawnFrames()
	oUF:RegisterStyle(addon.."Group5", Style)
	oUF:SetActiveStyle(addon.."Group5")
	local T = T
	local db = T.header
	local header = oUF:SpawnHeader(addon.."Group5", nil, 
	db.showSolo and "solo" or parent:GetVisibilityMacro(5), 
...
function module:OnInitialize()
	self.db = parent.db:RegisterNamespace("Group5", defaults)
	...
	self.frame = CreateFrame("Frame", "gUI4_GroupFrames_Group5_Scaffold", UIParent)
	self.frame.UpdatePosition = function() self:UpdatePosition() end
	self.fadeManager = LMP:NewChain(gUI4:CreateFadeManager("UnitFrames: Group5")) :SetFrameStrata("LOW") :SetFrameLevel(100) :Enable() .__EndChain
	self.overlay = gUI4:GlockThis(self.frame, COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATE5PLAYERS, function() return module.db.profile end, unpack(gUI4:GetColors("glock", "unitframes"))) 
