...
-- make into AceBucket.
-- @usage
-- MyAddon = GP_LibStub("GP_AceAddon-3.0"):NewAddon("BucketExample", "GP_AceBucket-3.0")
-- 
...
local MAJOR, MINOR = "GP_AceBucket-3.0", 3
local AceBucket, oldminor = GP_LibStub:NewLibrary(MAJOR, MINOR)
...
-- GLOBALS: GP_LibStub, geterrorhandler
...
local function RegisterBucket(self, event, interval, callback, isMessage)
	...
	if not AceEvent or not AceTimer then 
		AceEvent = GP_LibStub:GetLibrary("GP_AceEvent-3.0", true)
		AceTimer = GP_LibStub:GetLibrary("GP_AceTimer-3.0", true)
		if not AceEvent or not AceTimer then
...
-- @usage
-- MyAddon = GP_LibStub("GP_AceAddon-3.0"):NewAddon("MyAddon", "GP_AceBucket-3.0")
