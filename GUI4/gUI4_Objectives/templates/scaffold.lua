...
gUI4_Objectives.Scaffold = Scaffold

function Scaffold:ApplySettings()
	-- update position
	-- LibWin.RegisterConfig(self, self.settings.position)
	-- self:SavePosition()
	-- self:LoadPosition()
end
...
function Scaffold:New(id, name, settingsFunc, template)
	...
	local bar = setmetatable(template or CreateFrame("Frame", "GUI4GeneralCaptureBar"..numbars, UIParent), Scaffold_MT)
	bar.name = name or id
	bar.id = id
	bar.GetSettings = settingsFunc or function() end
	bar:SetSize(1,1)
