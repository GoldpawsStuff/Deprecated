...
local module = parent:NewModule("Focus", "GP_AceEvent-3.0")
...
local function scaffolding(self, unit)
	...
	self:RegisterEvent("PLAYER_FOCUS_CHANGED", function(self)
		...
		if Auras.createdIcons then
			for i = 1, Auras.createdIcons do
				local button = type(Auras[i]) == "table" and Auras[i]
				if button and button.StopFlash then
					button:StopFlash()
					button:StopAllFades()
					button:RawHide()
...
function module:OnInitialize()
	self.db = parent.db:RegisterNamespace("Focus", defaults)
...
function module:OnEnable()
	if not self.loaded then
		self:AddUnit("focus", scaffolding, self.db.profile)
		self.loaded = true
	end
end
