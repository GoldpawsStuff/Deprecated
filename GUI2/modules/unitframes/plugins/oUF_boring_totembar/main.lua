...
local PLAYER_TOTEM_UPDATE = function(self, event, slot)
	...
	local _, name, _, duration, _ = GetTotemInfo(slot)
...
local enable = function(self, unit)
	...
	if tb then
		...
		tb:SetScript("OnEvent", function(self, event, ...)
			if (self[event]) then
				self[event](orig, event, ...)
			else
				update(orig, event)
			end
		end)
		
		tb.PLAYER_TOTEM_UPDATE = PLAYER_TOTEM_UPDATE
		
		tb:RegisterEvent('PLAYER_TOTEM_UPDATE')
		tb:RegisterEvent('PLAYER_ENTERING_WORLD')
		tb:RegisterEvent('PLAYER_TALENT_UPDATE')
...
local disable = function(self)
	if self.TotemBar then
		self.TotemBar:UnregisterEvent('PLAYER_TOTEM_UPDATE')
		self.TotemBar:UnregisterEvent('PLAYER_ENTERING_WORLD')
		self.TotemBar:UnregisterEvent('PLAYER_TALENT_UPDATE')
	end
end
