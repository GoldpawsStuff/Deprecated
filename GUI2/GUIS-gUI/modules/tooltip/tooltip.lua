...
module.ToggleLock = function(self)
	if (Tooltip.isLocked) then
		self:Unlock()
	else
		self:Lock()
	end
...
module.Reset = function(self)
	self:Lock()
	Tooltip:ClearAllPoints()
	Tooltip:SetPoint(getDefaultPosition())
end
]]--
...
module.OnInit = function(self)
	...
	--[[
	--
	-- Create our holderframe, for user positioned tooltips
	-- This can't be done with gFrameHandler-3.0, as its position needs to change OnShow
	-- This is because in its default position, it is dependant on visible sidebars
	Tooltip = CreateFrame("Frame", self:GetName() .. "Frame", UIParent)
	Tooltip:Hide()
	Tooltip:SetFrameStrata("DIALOG")
	Tooltip:SetSize(300, 200)
	Tooltip:SetPoint(unpack(GUIS_DB["tooltip"].userpos))
	Tooltip:SetBackdrop(M["Backdrop"]["Blank"])
	Tooltip:SetBackdropColor(1, 1, 1, 1/3)
	Tooltip:SetHitRectInsets(-15, -15, -15, -15)
	Tooltip:EnableMouse(true)
	Tooltip:SetMovable(true)
	Tooltip:SetUserPlaced(true)
	Tooltip:RegisterForDrag("LeftButton", "RightButton")

	Tooltip.isLocked = true
--	Tooltip.isDefaultPosition = true
	
	local TooltipTitle = Tooltip:CreateFontString(nil, "OVERLAY")
	TooltipTitle:SetPoint("CENTER", Tooltip, "CENTER")
	TooltipTitle:SetFontObject(Tooltip_Med)
	TooltipTitle:SetText("Default Tooltip")
	TooltipTitle:SetTextColor(1, 1, 1, 1)
	...
	Tooltip:SetScript("OnDragStop", function(self) 
		...
		GUIS_DB["tooltip"].userpos = { GetPoint(self) }

		Tooltip.isDefaultPosition = false
	end)
