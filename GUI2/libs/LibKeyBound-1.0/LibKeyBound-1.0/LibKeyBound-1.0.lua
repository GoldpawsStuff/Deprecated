...
Revision: $Rev: 100 $
Author(s): Gello, Maul, Toadkiller, Tuller, Goldpaw
...
local MINOR = tonumber(("$Revision: 100 $"):match("(%d+)")) + 90000

--[[
	LibKeyBound-1.0
		ClickBinder by Gello and TrinityBinder by Maul -> keyBound by Tuller -> LibKeyBound library by Toadkiller -> Fix related to multiple editboxes by Goldpaw - Revision 100
...
function LibKeyBound.Binder:OnKeyDown(key)
	...
	if openChatKey and key == openChatKey then
		local f = ChatEdit_GetActiveWindow() -- fix by Goldpaw, Revision 100, was ChatFrameEditBox:Show() in revision 98/99
		if (f) then 
			f:Show() 
			return 
		end 
