...
local SECURE = {
	...
	Player_SecureCallback = [=[
		...
		elseif (name == "change-enablefrozendeathcrystal") then
			local owner = self:GetFrameRef("Owner");
			self:SetAttribute("enableFrozenDeathCrystal", value);
			if (value) then
				owner:CallMethod("EnableFrozenDeathCrystal");
			else
				owner:CallMethod("DisableFrozenDeathCrystal");
			end
		end
	]=],
...
UnitFramePlayer.OnInit = function(self)
	...
	if (IsWrath) then
		self.frame.EnableFrozenDeathCrystal = function()
			self.frame.Power:SetStatusBarTexture(self.layout.PowerBarFrozenDeathTexture)
			self.frame.Power.Bg:SetTexture(self.layout.PowerBackgroundFrozenDeathTexture)
			self.frame.Power.OverrideColor = function()
				--self.frame.Power:SetStatusBarColor(.9, .9, .9)
				self.frame.Power:SetStatusBarColor(.9, .9, .9, .85)
			end
		end
		self.frame.DisableFrozenDeathCrystal = function()
			self.frame.Power:SetStatusBarTexture(self.layout.PowerBarTexture)
			self.frame.Power.Bg:SetTexture(self.layout.PowerBackgroundTexture)
			self.frame.Power.OverrideColor = self.layout.PowerOverrideColor
			self.frame.Power:ForceUpdate()
		end
		if (self.db.enableFrozenDeathCrystal) then
			self.frame:EnableFrozenDeathCrystal()
		else
			self.frame:DisableFrozenDeathCrystal()
		end
	end
