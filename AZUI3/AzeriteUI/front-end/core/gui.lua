	...
	if (IsAzerite) then
		if (Core:IsModuleAvailable("UnitFramePlayer")) then
			...
			if (IsWrath) then
				...
				-- Player Frozen Death Crystal
				table_insert(PlayerMenu.buttons, {
					enabledTitle = L_ENABLED:format(L["Use Frozen Crystal"]),
					disabledTitle = L_DISABLED:format(L["Use Frozen Crystal"]),
					type = "TOGGLE_VALUE",
					configDB = "UnitFramePlayer", configKey = "enableFrozenDeathCrystal",
					proxyModule = "UnitFramePlayer"
				})
